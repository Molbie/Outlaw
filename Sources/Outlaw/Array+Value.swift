...
public extension Array {
    public static func mappedValue<Element: Outlaw.Value>(from object: Any) throws -> [Element] {
...
public extension Array {
    public static func mappedValue<Element: Outlaw.Value, T>(from object: Any, with transform:(Element) throws -> T) throws -> [T] {
        guard let anyArray = object as? [Any] else {
            throw OutlawError.typeMismatch(expected: self, actual: type(of: object))
        }
    ...
    public static func mappedValue<Element: Outlaw.Value, T>(from object: Any, with transform:(Element) -> T?) throws -> [T?] {
        guard let anyArray = object as? [Any?] else {
            throw OutlawError.typeMismatch(expected: self, actual: type(of: object))
        }
